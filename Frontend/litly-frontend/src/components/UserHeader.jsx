...
function UserHeader() {
  ...
  return (
    <header className='w-full fixed md:z-10 z-20 h-16 border-b-1 border-[#dbe0eb] bg-white shadow-[0_4px_6px_-4px_rgba(0,0,0,0.3)]' >

      <nav className='md:h-16 md:top-0 flex z-20 flex-col md:flex-row md:justify-between items-center md:px-10 md:w-auto md:py-9 bg-[white] md:bg-none shadow-lg '>
        <div className="flex flex-row items-center px-5 justify-between w-full md:bg-none bg-white h-16 z-20 static topContent transition-all duration-300 ease-in-out ">
          ...
          <Menu onClick={openMenu} className=' md:opacity-0 z-20' />
        </div>
        <div className={`navMenu z-10 md:h-16 md:relative fixed md:top-0 flex flex-col md:flex-row items-center gap-10 bg-white md:bg-none md:w-auto w-full md:pb-0 pb-10 transition-all duration-300 ease-in-out md:shadow-none shadow-lg ${menuStatus?'top-14':'top-[-14rem]'}`}>
